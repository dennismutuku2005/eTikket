...
import { FiEye, FiEyeOff, FiLock, FiUsers, FiCamera, FiPlus, FiTrash2, FiLoader, FiEdit2, FiCheck, FiX } from "react-icons/fi";
...
const EMPTY_EDIT = { full_name: "", email: "", phone: "", role: "gate_staff", password: "" };
...
export default function OrganizerStaffPage() {
    ...
    try {
      await apiRequestAuth("/staff", session.token, {
        ...
        body: JSON.stringify({
          ...
          password_hash: form.password, // backend hashes this
          role: form.role,
        }),
      });
  ...
  async function handleEditSubmit(e) {
    ...
    try {
      ...
      // Only send password if the organizer entered a new one
      if (editForm.password.trim()) {
        payload.password_hash = editForm.password;
      }

      await apiRequestAuth(`/staff/${editingId}`, session.token, {
        method: "PUT",
    ...
    >
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-xl rounded-[24px] border border-[#ececec] bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-[#ececec] pb-4">
              ...
              <button
                ...
                className="rounded-full border border-[#ececec] bg-white p-2 text-[#6b6b70] hover:bg-[#f4f4f5]"
              >
                <FiX size={16} />
              </button>
            </div>
      ...
      {isEditOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-xl rounded-[24px] border border-[#ececec] bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-[#ececec] pb-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-[#f33959]">Edit staff member</p>
                <h3 className="mt-1 text-xl font-bold text-[#0f0f10]">Update staff details</h3>
                <p className="mt-1 text-xs text-[#6b6b70]">Leave password blank to keep the current one.</p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleEditSubmit} className="mt-5 space-y-4">
              ...
              <label className="block">
                <span className="text-sm font-bold text-[#0f0f10]">New password <span className="font-normal text-[#6b6b70]">(leave blank to keep current)</span></span>
                <div className="relative mt-1.5">
                  <input
              ...
              <div className="flex flex-col gap-2 pt-2 sm:flex-row sm:justify-end">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-full bg-[#f33959] px-5 py-2.5 text-sm font-bold text-white transition hover:bg-[#d92847] disabled:opacity-50"
                >
                  {isUpdating ? <FiLoader size={14} className="animate-spin" /> : <FiCheck size={14} />}
